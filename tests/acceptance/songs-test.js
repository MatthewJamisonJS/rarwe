...
module('Acceptance | songs', function (hooks) {
  ...
  test('Sort songs in various ways', async function (assert) {
    ...
    assert
      .dom('[data-test-rr=song-list-item]:first-child')
      .hasText('Spinning n Daffodils', 'The first song is the highest rated');
    assert
      .dom('[data-test-rr=song-list-item]:last-child')
...
/*There is a one-to-one mapping between the query parameter in the URL and the corresponding property on the controller.
A binding is automatically set up between these two that keeps them in sync.
Because each route has its corresponding controller, the list of query parameters can also be defined on the route.
Because I think the concept of query parameters primarily relates to routing, I prefer to define them on the route.

Routes - need - Controllers (top-level components for a specific route)
*/
